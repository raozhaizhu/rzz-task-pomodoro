...
export const cardInfoSchemaClient = z.object({
  id: z
    .number()
    .int()
    .min(0, { message: "The id number must be greater than 0" }),
  ...
  description: z
    ...
    .trim()
    .max(300, {
      message: "The length of description can't exceed 300 characters",
    })
    .optional(),
  ...
  remark: z
    .string()
    .trim()
    .max(200, { message: "The length of remark can't exceed 200 characters" })
    .optional(),
  tags: z
    .string()
    .trim()
    .max(35, { message: "The target of times be can't exceed 35 times" })
    .optional(),
    ...
    (set, get) => ({
      tasks: [
        ...
        },
        {
          ...
          title: "Title",
          description:
            "Description \n Icon: mins(work/break) & remark  ⬆️ \n Button: delete & edit & start  ⬇️",
          tags: "tag1 tag2 tag3",
      ...
      getLatestId: () => {
        ...
        const id = tasks.reduce(
          (acc, task) => (task.id > acc ? task.id : acc),
          0
        );
        return id;
      },
      getTask: (id: number) => {
        ...
        if (!task)
          return { success: false, message: "The task is not existed" };
        return {
          success: true,
          message: "The task has been found successfully",
          data: task,
        };
      },
      addTask: (cardInfo: CardInfoSchemaClient): TasksResponse => {
        ...
        if (get().checkIfTheTaskExist(id))
          return {
            success: false,
            message: "The title of task can't be repeated",
          };
        if (!get().checkIfTasksLengthLessThan30())
          return {
            success: false,
            message: "The length of the list of tasks can't exceed 30",
          };
        ...
        return {
          success: true,
          message: "The task has been added to the list successfully",
        };
      },
      ...
      editTask: (cardInfo: CardInfoSchemaClient): TasksResponse => {
        const { id } = cardInfo;
        if (!get().checkIfTheTaskExist(id))
          return { success: false, message: "The task has not been created" };
